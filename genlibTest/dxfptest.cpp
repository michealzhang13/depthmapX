...
TEST_CASE("DXF Parsing (spline)")
{
    ...
    {
        REQUIRE(spline.getVertex(static_cast<int>(i)).x == Approx(controlPoints[i].x).epsilon(EPSILON));
        REQUIRE(spline.getVertex(static_cast<int>(i)).y == Approx(controlPoints[i].y).epsilon(EPSILON));
    }
