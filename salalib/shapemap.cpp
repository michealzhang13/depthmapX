      ...
      for (size_t i = m_connectors.size() - 1; i != paftl::npos; i--) {
         ...
         for (size_t j = m_connectors[i].m_connections.size() - 1; j != paftl::npos; j--) {
            if (m_connectors[i].m_connections[j] == rowid) {
               ...
               if (conn_col != -1) {
                  m_attributes.decrValue(static_cast<int>(i),conn_col);
               }
            }
...
void ShapeMap::undo()
{
   ...
   if (event.m_action == SalaEvent::SALA_CREATED) {
      ...
      if (rowid != -1 && m_hasgraph) {
         ...
         size_t connectivity = getLineConnections(event.m_shape_ref,m_connectors[rowid].m_connections,TOLERANCE_B*__max(m_region.height(),m_region.width()));
         // update:
         int conn_col = m_attributes.getOrInsertLockedColumnIndex("Connectivity");
         m_attributes.setValue(rowid, conn_col, (float) connectivity );
         //
   ...
   if (poly.isClosed()) {
      ...
      for (size_t k = 0; k < poly.m_points.size(); k++) {
         ...
         for (size_t i = 0; i < pixels.size(); i++) {
            ...
            it->m_polyrefs.push_back(static_cast<short>(k));
               ...
               case SalaShape::SHAPE_LINE:
                  if (intersect_region(li,poly.m_region)) {
                     ...
                     if (intersect_line(li,poly.m_region,tolerance)) {
                        shapeindexlist.add(std::distance(m_shapes.begin(), shapeIter));
                     }
                  }
                  break;
               case SalaShape::SHAPE_POLY:
               ...
               for (const ShapeRef& shapeRef: shaperefs) {
                  if (*iter != shapeRef && ((iter->m_tags & ShapeRef::SHAPE_CENTRE) || (shapeRef.m_tags & ShapeRef::SHAPE_CENTRE))) {
                     if (testedlist.add(shapeRef.m_shape_ref) != -1) {
                        shapeindexlist.add(depthmapX::findIndexFromKey(m_shapes, (int) shapeRef.m_shape_ref));
                     }
                  }
               }
            }
...
size_t ShapeMap::getLineConnections(int lineref, pvecint& connections, double tolerance)
{
            ...
            try {
               ...
               if ( intersect_region(line, l, line.length() * tolerance) ) {
                  ...
                  if ( intersect_line(line, l, line.length() * tolerance) ) {
                     connections.add(depthmapX::findIndexFromKey(m_shapes, (int) shape.m_shape_ref));
                     num_intersections++;
                  }
               }
            }
...
size_t ShapeMap::getShapeConnections(int shaperef, pvecint& connections, double tolerance)
{
...
void ShapeMap::makeShapeConnections()
{
   if (m_hasgraph) {
      ...
      for (auto shape: m_shapes) {
         ...
         m_connectors.push_back( Connector() );
         size_t connectivity = getShapeConnections( key, m_connectors[i].m_connections, TOLERANCE_B*__max(m_region.height(),m_region.width()));
         m_attributes.setValue(rowid, conn_col, (float) connectivity );
      }
...
bool ShapeMap::setCurSel(const std::vector<int>& selset, bool add)
{
   ...
   for (size_t i = 0; i < selset.size(); i++) {
      ...
      if (index != paftl::npos) {
         if(m_selection_set.insert(static_cast<int>(index)).second) {
            m_attributes.selectRowByIndex(static_cast<int>(index));
         }
         shapeIter->second.m_selected = true;
         m_selection = true;
      }
...
bool ShapeMap::linkShapes(int id1, int dir1, int id2, int dir2, float weight)
{
   int success = -1;
   if (dir1 == 1) {
      success = static_cast<int>(m_connectors[id1].m_forward_segconns.add(SegmentRef(dir2,id2),weight));
   }
   else {
      success = static_cast<int>(m_connectors[id1].m_back_segconns.add(SegmentRef(dir2,id2),weight));
   }
